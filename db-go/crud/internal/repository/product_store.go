...
type RepositoryProductStore struct {
	...
	st domain.StoreProduct
}

// FindById finds a product by id.
func (r *RepositoryProductStore) FindById(id int) (p domain.Product, err error) {
	// read all products
	ps, err := r.st.ReadAll()
	if err != nil {
		return
...
func (r *RepositoryProductStore) Save(p *domain.Product) (err error) {
	// read all products
	ps, err := r.st.ReadAll()
	if err != nil {
		return
...
func (r *RepositoryProductStore) UpdateOrSave(p *domain.Product) (err error) {
	// read all products
	ps, err := r.st.ReadAll()
	if err != nil {
		return
...
func (r *RepositoryProductStore) Update(p *domain.Product) (err error) {
	// read all products
	ps, err := r.st.ReadAll()
	if err != nil {
		return
